...
<body>
	...
	<%
	Page<Comment> p = (Page<Comment>)session.getAttribute("page");
	//第一次访问就为null
	if(p==null){
		response.sendRedirect("../ShowCommentServlet?page=1");
	}else{
		//不是第一次就直接得到数据
		List<Comment> list = p.getData();
	...
	<div class="a">
		<a style="float: left">当前位置 :&nbsp;</a> <a>&nbsp;首页</a> <span
			class="glyphicon glyphicon-menu-right"></span> <a>酒店评论</a>
	...
	<div class="taskbar">
		...
		<div style="width: 1000px; margin-left: 200px; margin-top: -50px">
			<ul>
				<c:forEach var="comment" items="${list }" varStatus="i">
					<li class="li">
					<img src="<%=request.getContextPath()%>/upload/${comment.upicture }"
						class="img" />
					<span class="pingname">${comment.uname }</span> 
					<div class="date">
		...
		<div style="width: 120px; margin-left: 600px">
			<a href="../ShowCommentServlet?page=${p.current-1 }">上一页 </a>
			<span> ${p.current }/${p.totlePage } </span>
			<a href="../ShowCommentServlet?page=${p.current+1 }"> 下一页</a>
		</div>
		<div style="width: 100px; margin-left: 195px; font-size: 25px">评论</div>
		<div style="width: 1000px; height: 150px; margin-left: 190px">
			<textarea
				style="width: 90%; height: 100%; padding-bottom: 110px; margin-left: 10px"></textarea>
		</div>

		<button
			style="width: 150px; margin-left: 950px; height: 35px; background-color: #009966; color: white; font-size: 17px; border-radius: 8px; line-height: 10px; cursor: pointer; margin-top: 10px;">发表</button>
			...
			$("#b").click(
					function() {
						...
						if (text == "编辑") {
							...
							$(".read").css("border", "none").css("outline",
									"none").attr("readOnly", " ");
							var a = $("#b").text("编辑");
